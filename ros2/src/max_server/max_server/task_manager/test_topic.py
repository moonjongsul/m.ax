...
class TaskManagerNode(Node):
    def __init__(self):
        ...
        # ============ 콜백 그룹 분리 ============
        # I/O 그룹: 구독 + 버퍼링 (서로는 직렬 실행, 추론과는 병렬)
        self.io_group = MutuallyExclusiveCallbackGroup()
        # 추론 그룹: 모델 추론 전용 (I/O와 독립적인 스레드에서 실행)
        ...
        # ---------- 추론 그룹 ----------
        self.load_model()
        self.inf_timer = self.create_timer(
            1.0,
    ...
    def img_callback(self, msg: CompressedImage):
        with self.latest_msg_lock:
            self.latest_msg = msg
        self.get_logger().info(f"img receive")
    ...
    def buffer_callback(self):
        ...
        with self.buffer_lock:
            ...
            buf_len = len(self.img_buffer)

        self.get_logger().info(f"buffer: {buf_len}")

        # 디버그용 시각화 publish 필요하면 주석 해제
        # if buf_len == IMG_BUFFER_LEN:
        #     with self.buffer_lock:
        #         frames_snapshot = list(self.img_buffer)
        #     self.show_frames(frames_snapshot)
    ...
    def inference_callback(self):
        ...
        # 버퍼 스냅샷만 빠르게 떠서 빠져나옴 — 추론 중에도 버퍼 갱신 계속됨
        with self.buffer_lock:
            if len(self.img_buffer) < IMG_BUFFER_LEN:
                return
            frames = list(self.img_buffer)
        ...
        self.get_logger().info(
            f"============== [{self.inf_count}] Total: {elapsed:.2f} sec ============="
        )
    ...
    def show_frames(self, frames):
        ...
        ok, encoded = cv2.imencode(".jpg", canvas, encode_params)
        if ok:
            msg = CompressedImage()
...
def main(args=None):
    ...
    # I/O 그룹용 1, 추론 그룹용 1, executor 자체 여유분 1
